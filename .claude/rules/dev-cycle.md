...
| コナセンス | 連動箇所 | 影響 |
|-----------|---------|------|
| **opusレビューパターン** | plan(セルフレビュー), architect(品質検証), breakdown(タスク分解レビュー), implement(レビューサイクル), review-fix(レビュー) の5箇所 | 「コンテキスト分離でopusに委譲」という共通原則。廃止すると生成バイアスが混入。ただしレビュー観点は各スキル固有（設計品質 vs コード品質）で統一不要 |
