...
**フラグ基準**:
- 「難」タスクが分割可能な独立した変更を含んでいる
- 「易」だが複数コンポーネントにまたがる変更を含む

**改善**: 難易度を変更範囲・複雑さの実態に合わせて修正。「難」を超える場合はサブタスクに分割。
...
- plan のフェーズ受け入れ条件が Phase の完了基準に転記されていない
- タスクの成功基準が Phase の受け入れ条件に寄与しない（孤立タスク）
...
- 実行前に確認できない固定値を使っている（例: 「2件が出力される」←事前確定不可）

**改善例**:
- 「適切にエラーハンドリング」→「400/404/500を返しエラーメッセージに原因を含む」
- 「正しく動作する」→「入力Xに対して出力Yを返す」
- 事前確定不可の数値 → 範囲表現（「2件」→「1件以上」、「N件増加」→「実行前より増加」）
