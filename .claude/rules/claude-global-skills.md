...
- **スクリプト言語選択基準**: CLIコマンド合成→シェル、ロジック含む処理→Python（PEP 723 + uv run）。詳細は skill-design-patterns.md「スクリプト設計の要点」参照
- **スクリプト配置**: 複数スキル共有→`skills/scripts/`、スキル固有→各スキルの`scripts/`
- **スクリプト出力の終端判定キー**: quality-gate.py は JSON `gate` フィールド (`PASS`/`FAIL`/`SKIP`)、changed-files.sh は `RESULT: NO_CHANGES/TOO_LARGE/PROCEED` 等テキスト。SKILL.mdが判定キーで分岐
