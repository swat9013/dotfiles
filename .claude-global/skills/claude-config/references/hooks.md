...
## TOC
1. [イベント完全一覧（25種）](#イベント完全一覧) / 2. [Hook 設定フィールド](#hook-設定フィールド) / 3. [PreToolUse 権限決定の優先度](#pretooluse-権限決定の優先度) / 4. [hookSpecificOutput 仕様](#hookspecificoutput-仕様) / 5. [stdin JSON 構造](#stdin-json-構造) / 6. [実装パターン](#実装パターン) / 7. [環境変数](#環境変数) / 8. [デバッグ・段階的導入](#デバッグ段階的導入)
...
| `PreCompact` | 可 | `context_size`, `token_count` |
...
### `once` / `async` / `asyncRewake` フィールド

```json
{ "type": "command", "command": "...", "once": true }
{ "type": "command", "command": "...run-tests-async.sh", "async": true, "timeout": 300 }
{ "type": "command", "command": "...monitor.sh", "asyncRewake": true }
```

`once`: セッション中1回のみ（初期化向け）。`async`: バックグラウンド実行、stdout に `{"systemMessage":"..."}`。`asyncRewake`: `async: true` 含意。exit 2 でアイドル時に Claude を即起動し stderr（空なら stdout）をシステムリマインダーとして渡す。長時間バックグラウンド処理の失敗検知向け。
...
PreToolUse とは別イベント。`decision` に `behavior`（`"allow"`/`"deny"`、PreToolUse の `permissionDecision` とは別名）、`updatedInput`（allow時、全置換）、`updatedPermissions`（allow時、session permission更新配列）、`message`（deny時の理由）、`interrupt`（deny時 `true` で Claude 停止）を含む。`behavior: "allow"` は `permissions.deny` / `ask` ルールを上書きしない。`updatedInput` は deny/ask に再評価される（v2.1.110+）ため、入力置換による permission rules 迂回は不可。
...
**SessionEnd タイムアウト（v2.1.105+）**: デフォルト 1.5 秒。per-hook `timeout` 設定時は settings 内 per-hook timeout 最大値まで予算が自動引き上げ（上限 60 秒、プラグイン提供フックは対象外）。明示的上書きは `CLAUDE_CODE_SESSIONEND_HOOKS_TIMEOUT_MS`（ミリ秒）。
**PreCompact ブロッキング（v2.1.105+）**: exit 2 または `{"decision":"block"}` でブロック。プロアクティブ実行（制限前）→ スキップし会話継続。回復実行（制限エラー後）→ 基礎エラーがサーフェスされリクエスト失敗。
**PermissionRequest `setMode: bypassPermissions` の有効条件**: セッション起動時に bypass mode 利用可（`--dangerously-skip-permissions` / `--permission-mode bypassPermissions` / `--allow-dangerously-skip-permissions` / `defaultMode: "bypassPermissions"`）かつ `permissions.disableBypassPermissionsMode` 未設定時のみ効果あり。それ以外は no-op、`defaultMode` として永続化もされない。
