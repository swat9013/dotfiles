...
|-------|---------|--------------|---------------------|---------|
| 1 | 設定した自動化戦略（hooks/permissions）が意図通りに機能しているか。手動承認の頻出パターンから自動化の穴を特定する | `scripts/scan-hooks.py`, `scripts/scan-metrics.py` | `references/hooks.md`, `references/settings.md` | - |
| 2 | 推論的コンテキスト（CLAUDE.md/rules/memory）が Claude の行動を目的通りに方向づけているか。memory の内容が適切な永続化先に配置されているかも含む | `scripts/scan-claude-md.py` | `references/claude-md.md` | → 昇華チェック（下記） |
