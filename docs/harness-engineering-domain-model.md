...
| Tier 2 | hook追加 → scan-metrics.py（claude-config Agent 1） | PermissionRequest回数, permission_mode別分布, allow_ineffective, ユーザーツール拒否回数 |

#### Tier 2 拡張運用規範（Computational First の運用）

診断 Agent が Tier 2 出力で不足する集計を必要とした場合、**インラインコード実行（`python -c` 等）で補完してはならない**。代わりに以下のフローを取る:

1. Agent は「scan-metrics.py に X が欠落している」として改善提案テーブルに含める（report-forward）
2. harness-tuning 側で Cat-1（スクリプトの出力不足）として script 拡張を適用
3. 次回診断で追加集計が利用可能になる

この規範により Tier 2 は「現行スクリプト出力の範囲内」に閉じ、診断実行のたびの差分（再現性低下）と任意コード実行経路（セキュリティ劣化）を排除する。違反は PreToolUse ガードで物理的に遮断する（Feedforward + Feedback の二層防御）。
