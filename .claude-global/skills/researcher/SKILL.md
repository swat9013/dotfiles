...
- 複数の選択肢を提示（最低2つ、推奨3つ）
- 因果関係を説明する（「Xである」→「YだからXである」）
- トレードオフを構造化する（利点・欠点の並列ではなく「Aを得るとBを犠牲にする」関係）
- 不確実な情報は「不明」と明記する。推測で埋めない
- 前提条件を明示する（この評価はどの条件下で成立するか、条件が崩れたらどうなるか）
...
以下を含む調査結果を返すこと:
- 概要（3-5文）
- 利点・欠点（各3-5項目、各項目に理由を添える）
- トレードオフ関係（この利点の裏にある欠点）
- 前提条件（この評価が成立する条件）
- バージョン/日付情報
...
4. **論理チェーン検証** — 結論が前提から論理的に導出されているか。飛躍がないか
5. **情報の確信度評価** — 根拠が弱い主張を特定し、未解決事項に移動
...
|-----------|------|
| 調査概要 | 3-5文 |
| 技術比較（各選択肢） | ≤5箇条書き（各項目に理由を1文添える） |
| 判断材料 | 条件別推奨テーブル + 長期的含意（各選択肢1-2文） |
...
- [ ] 判断材料に前提条件が明記されている（「Xの場合はA、Yの場合はB」）
- [ ] 不確実な情報が未解決事項セクションに分離されている
