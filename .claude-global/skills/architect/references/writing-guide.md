...
plan.md は「戦略文書」— 何をなぜ変えるか + 成功条件。
implementation.md は「実行計画」— 具体的な手順とメソッド名。

| 適切な記述 | 改善が必要な記述 |
|-----------|---------------|
| 「ドロップダウン選択肢を再利用方式に変更（`page.py` の生成ロジック改修）」 | 「`_build_options()` → `_compute_entries()` に改名、`list[tuple]` を返す純粋関数に」 |
| 「画面全体更新→変更コントロール個別更新に切替」 | 「`self._shared_options: list[Option] = []` を追加し `dd.update()` を各所に挿入」 |
| 「認証フローを同期→非同期に変更」 | 「`async def authenticate()` に変更し `await` で呼び出す」 |

**判断基準**: 方針説明の文でメソッド名が主語・述語 → ドメイン言語での言い換えを先に追加。ファイルパス + 変更方向の補足としてのメソッド名 → OK。
...
| 過剰な詳細 | メソッド名が方針説明の主軸になり、ドメイン言語での本質記述がない | 設計書は What/Why（ドメイン言語で先に述べ、メソッド名は補足）。How の詳細は breakdown/implement に委譲 |
