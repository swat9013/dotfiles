...
| **scan スクリプト分割** | scan-config.py（共通コンテキスト・encoded_cwd）↔ scan-hooks.py + scan-metrics.py（Agent 1、セッション定量含む）↔ scan-claude-md.py（Agent 2）↔ scan-skills.py（Agent 3）↔ memory/（Agent 4） | スクリプト追加/移動時に SKILL.md の Agent テーブルと不整合。scan-config.py の出力は全 Agent に共有される前提。Agent 4 追加時も SKILL.md の Agent テーブルを更新 |
