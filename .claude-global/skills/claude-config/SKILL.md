...
Bash tool で scan-config.sh を実行し、.claude/ 構成と project-context を取得する。
...
|-------|--------|--------------|---------------------|
| 1 | settings.json + hooks 検証 | `scripts/scan-hooks.sh` | `references/hooks.md` |
| 2 | CLAUDE.md + rules/ 検証 | `scripts/scan-claude-md.sh` | `references/claude-md.md` |
| 3 | skills/ + .claude/ 構成検証 | `scripts/scan-skills.sh` | `references/skills.md`, `references/skill-design-patterns.md` |
...
`Read ~/.claude/skills/claude-config/references/upstream-sources.md` で `last-auto-update` 日付と、固定URL・検索クエリ・監視対象ワークアラウンドを把握する。
...
`upstream-sources.md` の `last-auto-update` を本日日付（YYYY-MM-DD）に更新 → 共通レビューフェイズ（R1〜R3）へ
