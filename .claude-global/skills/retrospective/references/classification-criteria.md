...
**具体例**: rules/ の paths は YAML配列形式・クォート付きでは動作しない (#21858)／sheldon のプラグイン設定は TOML 形式、インラインコメント非対応
...
**具体例**: rules/ の paths はカンマ区切り・クォートなし形式でのみ動作する→記法に注意／サブエージェント（Task tool）は親コンテキストを消費しない→重い分析はサブエージェントに委譲
