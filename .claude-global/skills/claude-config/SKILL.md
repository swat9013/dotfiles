...
|-------|--------|--------------|---------------------|
| 1 | settings.json + hooks 検証 | `scripts/scan-hooks.py`, `scripts/scan-metrics.py` | `references/hooks.md` |
...
| 4 | memory/ 昇華チェック | - | `references/claude-md.md`（昇華判定基準） |

Agent 1 は `scan-hooks.py` に加えて `scan-metrics.py` も実行する。`scan-metrics.py` の出力（tool 別 PermissionRequest カウント）は permissions.allow / guard との相関分析に使用する。

**各エージェントへの共通指示**:
1. スクリプトがある場合は実行して PASS/WARN 結果を取得（Agent 4 はスキップ）
...
4エージェントの出力を統合し、証拠なき指摘を除外して優先度（Critical/High/Medium/Low）を割り当てる:

`| # | 優先度 | カテゴリ | 問題 | 改善案 |` の表形式で出力。Agent 4 のメモリ昇華候補も改善提案テーブルに含める。
