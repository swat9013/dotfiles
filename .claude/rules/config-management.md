...
| **scan スクリプト分割** | scan-config.py（共通コンテキスト）↔ scan-hooks.py（Agent 1）↔ scan-claude-md.py（Agent 2）↔ scan-skills.py（Agent 3） | スクリプト追加/移動時に SKILL.md の Agent テーブルと不整合。scan-config.py の出力は全 Agent に共有される前提 |
