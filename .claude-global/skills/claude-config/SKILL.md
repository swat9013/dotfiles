...
Bash tool で2つのスクリプトを並列実行し、構造化テキスト出力を取得する。

```
~/.dotfiles/.claude-global/skills/claude-config/scripts/scan-config.sh
~/.dotfiles/.claude-global/skills/claude-config/scripts/scan-skills.sh
```

出力が空や異常な場合は、スクリプトの存在・実行権限を確認する。scan-skills.sh の出力は Agent 3 に渡す。
...
各エージェントには、ロール・フォーカス範囲・フラグ条件・出力形式・Step 1 のスキャン出力・上記 references 内容・**scan-config.sh が出力する構造ヒントセクション（project-context, hooks-scripts, rules-paths-check）の参照指示**・**scan-skills.sh の出力（Agent 3 用）** を渡す。
...
- scan-skills.sh の出力（WARN/ALERT 項目）を起点に詳細調査
...
Task tool (sonnet, allowed-tools: WebSearch, WebFetch, Write, Bash):
  prompt: |
    ...
    ## 出力
    1. 調査結果を `.claude/research/YYYY-MM-DD-HHMMSS-claude-code-updates.md` に Write
