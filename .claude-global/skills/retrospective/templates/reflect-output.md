...
| 2 | 技術選択 | TypeScriptのstrictモードを採用。コンパイル時型エラー検出を優先し、実行時エラーを最小化 | CLAUDE.md | 中 |
...
| 2 | コーディング規約 | このプロジェクトではAPIレスポンスのエラー処理をResult型で統一。例外throwは禁止 | rules/ | 中 |
...
| 2 | 依存関係 | このプロジェクトではNode.js 20以上が必要。18系ではfetch APIがexperimentalのため動作保証外 | CLAUDE.md | 中 |
