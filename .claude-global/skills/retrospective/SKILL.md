...
- 矛盾する知見は両方記載し矛盾を明記
- 知見が0件なら「知見なし」とだけ出力
- retro-data.json に `metrics_summary` キーが存在する場合、`tool_counts` を確認し、PermissionRequest が多発しているツールが逸脱パターン（permissions.allow 未登録・guard 抜け等）かを評価する
...
150行超: 「CLAUDE.md が150行を超えました。`/claude-config` の実行を検討してください。」
...
- CLAUDE.md: X件の知見を追記
- rules/: Y件の提案（Z件承認済み）
- skills/: W件の提案（実行は /skill-creator で）
