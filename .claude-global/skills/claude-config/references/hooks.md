...
`asyncRewake`: `async: true` を含意。exit code 2 で終了するとアイドル時でも即座に Claude を起動。フックの stderr（空なら stdout）がシステムリマインダーとして Claude に渡される。長時間バックグラウンド処理の失敗を検知して Claude に反応させる用途向け。
...
**SessionEnd タイムアウト（v2.1.105+）**: デフォルト 1.5 秒。per-hook `timeout` を設定した場合、settings ファイル内の per-hook timeout 最大値まで自動的に予算が引き上げられる（上限 60 秒）。プラグイン提供フックの timeout は予算を引き上げない。明示的な上書きは `CLAUDE_CODE_SESSIONEND_HOOKS_TIMEOUT_MS` 環境変数（ミリ秒）。
**PreCompact ブロッキング（v2.1.105+）**: `PreCompact` で exit code 2 または `{"decision":"block"}` を返すとコンパクションをブロック可能。挙動はトリガー種別で異なる: プロアクティブ実行（コンテキスト制限前）→ スキップして会話をコンパクション未実行のまま継続、回復実行（コンテキスト制限エラー後）→ 基礎エラーがサーフェスされ現在のリクエストが失敗。
