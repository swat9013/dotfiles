...
**設計判断時の問い**: Phase 2 の設計案に対し `~/.claude/skills/_shared/design-questions.md` の該当フェーズの問いを適用する。適用ルールに従うこと。
...
- prompt: |
    ...
    ## 報告の書き方
    - コンポーネントの責務と依存関係をドメイン言語・一般技術用語で報告する
    - 再利用可能な既存実装があれば特に注目して報告する
    - メソッド名・シグネチャは「参考箇所」として補足に留め、羅列しない
```
...
- prompt: |
    ...
    ## タスク
    上記コンテキストに基づき、**戦略レベル**の実装計画を設計する。
    plan.md は「何をなぜ変えるか」の戦略文書。「どう実装するか」の詳細は breakdown/implement に委譲する。

    - 設計概要（なぜこのアプローチか）
    - 影響を受ける主要コンポーネントと変更方向（パス付き）
    - 実装フェーズ（各フェーズの目的・受け入れ条件・品質ゲート）
    - 再利用すべき既存実装（パス付き）
    - トレードオフ（何を優先し何を妥協したか）

    **記述の優先順位**: ドメイン言語・一般技術用語で方針を先に述べ、メソッド名は「どこを触るか」の補足として使う。
    - OK: 「ドロップダウン選択肢の再利用方式に変更（`settings/page.py` の既存生成ロジックを改修）」
    - NG: 「`_build_font_dropdown_options()` を `_compute_option_entries()` に改名し `list[tuple[str, str]]` を返す純粋関数に変更」
```
...
- **実装フェーズ**: 各フェーズの目的と受け入れ条件。以下のテンプレートで記述:
  ```
  ### Phase N: [フェーズ名]
  **目的**: [ドメイン言語で何が変わるか]
  **受け入れ条件**: [検証可能な完了基準]
  **主な変更箇所**: [ファイルパス + 変更方向の補足]
  ```
- **検証方法**: どうテスト・確認するか
...
- prompt: |
    ...
    3. フェーズの受け入れ条件と戦略的明快さ — 各フェーズの「完了した状態」が検証可能な形で書かれているか。方針説明でメソッド名が主語・述語になりドメイン言語での言い換えがない箇所はフラグ
    4. 参照の正確性 — ファイルパスが実在するか、既存実装の参照が正しいか
    5. 検証方法の妥当性 — テスト・確認方法が完了の定義をカバーしているか
    6. 設計判断の問い — `~/.claude/skills/_shared/design-questions.md` の適用ルールに従い、plan内の設計判断に該当する問いのみ選択して適用。検出パターンに該当する箇所があれば深掘り方向に従って展開する
