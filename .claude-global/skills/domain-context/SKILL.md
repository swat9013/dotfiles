---
name: domain-context
description: プロジェクト固有のドメイン知識を抽出・永続化する。「学びを保存」「知見を保存」「ドメイン知識更新」「domain-context」「ドメインコンテキスト」「振り返り」「reflect」「知見を記録」「知見をまとめて」「セッションから学ぶ」「CLAUDE.mdに追記」「CLAUDE.md更新」「プロジェクト知識」「コンテキスト記録」「プロジェクトの学び」「今の学びを残して」と依頼された時に使用。
...
| `templates/adr.md` | ADR（アーキテクチャ決定記録）テンプレート |
