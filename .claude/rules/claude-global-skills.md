...
**fork 適性**: サブエージェント不使用 + コンテキスト消費大 + ファイル契約で入出力が繋がる
**fork 不適**: 内部でAgent/Task tool使用 / 対話型 / 直前の会話を分析対象とする
...
- **スクリプト**: 言語（CLI合成→シェル、ロジック→Python PEP 723）、配置（共有→`skills/scripts/`、固有→各`scripts/`）、詳細は skill-design-patterns.md 参照
- **スクリプト判定キー**: quality-gate.py→JSON `gate`(`PASS`/`FAIL`/`SKIP`)、changed-files.sh→`RESULT: NO_CHANGES/TOO_LARGE/PROCEED`。品質ゲートは環境自動検出+`--lint-cmd`オーバーライド
