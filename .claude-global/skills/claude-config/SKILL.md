...
現在のプロジェクトの memory/ ディレクトリが存在する場合、MEMORY.md と各トピックファイルを Read してメモリ内容を把握する。パスは `~/.claude/projects/<encoded-cwd>/memory/`（encoded-cwd は cwdの絶対パスをハイフン区切りにしたもの、例: `-Users-s-watanabe--dotfiles`）。

### Step 2: 4並列サブエージェント分析

4エージェントを単一メッセージの Task tool で foreground 並列起動（sonnet）する。各エージェントに Step 1 の共通コンテキスト出力を渡す。
...
| 4 | tmp/review/ 頻出指摘抽出 | `scripts/scan-review.py` | `references/review-criteria.md` |

Agent 4 は以下を実行する:
1. scripts/scan-review.py を実行して frequent_dimensions と warn_count を取得
2. references/review-criteria.md を Read して dimension 定義を把握
3. frequent_dimensions が空でない場合、昇格候補として報告する
...
4エージェントの出力を統合し、証拠なき指摘を除外して優先度（Critical/High/Medium/Low）を割り当て:

`| # | 優先度 | カテゴリ | 問題 | 改善案 |` の表形式で出力。

**メモリ昇華チェック**: Step 1 で読み込んだメモリを確認し、昇華候補があれば改善提案テーブルに追加する。昇華判定基準: CLAUDE.mdに同内容が既にある→削除候補、複数セッションで繰り返し参照→昇華候補、一時的作業メモ→維持。
