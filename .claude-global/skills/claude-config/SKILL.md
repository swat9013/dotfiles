...
| `references/` | hooks, claude-md, skills（診断モード）、subagent, task-management, context-architecture（自己改善モード） | スキル実行時に参照 |
...
診断モード Step 5 / 自己改善モード U4 の変更適用後に実行する。
...
```bash
~/.dotfiles/.claude-global/claude-output-init.sh review
```
