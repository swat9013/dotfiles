...
- `~/.claude/projects/` をファイルシステム走査し、git管理外の削除済みworktreeを含む関連セッションを収集（各セッションに `session_dir_name`（エンコード済みディレクトリ名）を付与）
- `--limit` は全セッションディレクトリ横断での新しい順に適用
- subagent（Plan/Explore等ワークフローセッション）は収集対象外。親セッションに返った最終成果物と `tmp/review/*.md` で代替する

stdout にサマリー（total, skipped, session_dirs, セッション一覧）と `file=<パス>` が表示される。
...
1. Read tool で以下のファイルを読み込む（Bash や python3 は使わないこと）:
   ...
   - データファイルの review_files に含まれる各ファイルパス（tmp/review/ の独立レビュー結果 md ファイル）があれば Read する
2. データファイルの sessions 配列を、以下の6つの検出レンズで走査する
   - 各セッションの `session_dir_name` を確認し、複数セッションディレクトリを横断する知見（同一ブランチパターンの繰り返しなど）があれば検出する
...
claude-config が出力する診断ログ（`retrospective/output-schema.md`）を消費し、判断基準（reference）と価値判断（value）の語彙が誤ったフィールドに混入していないかを検出する。

**入力契約**:
- 入力: `~/.claude-global/skills/retrospective/output-schema.md` の JSONL レコード
- 使用フィールド: `reference_cited`・`value_stated`・`judgment_type`・`rationale`

**パターンA（reference→value誤写）**:
- `reference_cited` フィールドに本来 `value_stated` の内容（価値判断文字列）が混入している割合
- 検出方式: 空白区切りトークン単位
- 初期閾値: 混入率 **30%** 以上でフラグ（月次調整対象）

**パターンB（value→reference誤写）**:
- `value_stated` フィールドに本来 `reference_cited` の内容（アンカーID文字列 `#xxx-harness` 等）が混入している割合
- 検出方式: 空白区切りトークン単位
- 初期閾値: 混入率 **20%** 以上でフラグ（月次調整対象）

**C群観察項目**（構造的パターン検出）:
- 問題A: 目的関数の配置違反（親子目的関数非対称配置の誤用）
- 問題B: Agent表への追記漏れ（診断対象ドメイン一致原則未適用）
- memory昇格時の §2.x 参照整合（アンカー参照への書換忘れ）

**月次調整プロセス**:
1. retrospective 所有者が過検出/過小検出ログを確認し閾値変更提案
2. `claude-config/SKILL.md` で受け入れ判断（分担原則準拠）
3. 変更履歴を本セクション末尾に記録

<!-- 閾値変更履歴 -->
<!-- 2026-04-17: 初期値設定（パターンA=30%, パターンB=20%） -->
...
- 矛盾する知見は両方記載し矛盾を明記
- 知見が0件なら「知見なし」とだけ出力
- review_files に独立レビュー結果ファイルが含まれる場合、それらの指摘からも知見を抽出する（L3-L5 レンズが適用可能）
```
