...
| project（永続的な方針・制約） | CLAUDE.md or プロジェクトCLAUDE.md | 時限性がなく全般適用 | 昇格対象 |
...
- **目的関数**: `~/.claude/skills/_shared/independent-review-prompt.md` を Read し適用する（harness 本体レビューなので #architecture-fitness-harness が主目的関数として選択される。ハブは両分岐に同等に注入される共通ハブであり、対象に応じて分岐が選択される）。
