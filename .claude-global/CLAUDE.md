...
## 作業ルール
- plan mode で計画を書き終えたら、ExitPlanMode を呼ぶ前に目的と前提を整理して設計原則に基づく自己レビューを実施する。問題があれば修正してから提案する。
...
- シェルスクリプトは `chmod +x` で実行権限を付与し直接実行する（`./script.sh`）。`bash script.sh` や `sh script.sh` での実行は禁止。
